...
template <typename type>
vec<type> &vec<type>::add(const vec<type> &vec1, const vec<type> &vec2) {
...
template <typename type>
vec<type> &vec<type>::mult(type scalar, const vec<type> &vector) {
...
template <typename type>
mat<type> &mat<type>::add(const mat<type> &mat1, const mat<type> &mat2) {
...
template <typename type>
mat<type> &mat<type>::mult(const type scalar, const mat<type> &matrix) {
...
template <typename type>
mat<type> &mat<type>::mult(const mat<type> &mat1, const mat<type> &mat2) {
...
template <typename type>
vec<type> &vec<type>::operator+=(const vec<type> &other) {
...
template <typename type>
bool operator==(const vec<type> &vec1, const vec<type> &vec2) {
  if (vec1.size() != vec2.size()) {
    return false;
  }
...
template <typename type>
vec<type> operator+(const vec<type> &vec1, const vec<type> &vec2) {
...
template <typename type>
vec<type> operator*(const type &scalar, const vec<type> &vector) {
...
template <typename type>
vec<type> vec<type>::cross(const vec<type> &other) const {
...
template <typename type>
type operator*(const vec<type> &vec1, const vec<type> &vec2) {
  is_same_size(vec1, vec2);
...
template <typename type>
bool operator==(const mat<type> &mat1, const mat<type> &mat2) {
  if (mat1.rows() != mat2.rows() || mat1.columns() != mat2.columns()) {
    return false;
  }
...
template <typename type>
mat<type> &mat<type>::operator+=(const mat<type> &other) {
...
template <typename type>
mat<type> operator*(const mat<type> &mat1, const mat<type> &mat2) {
  is_same_size_mult(mat1, mat2);
...
template <typename type>
mat<type> operator*(const type scalar, const mat<type> &matrix) {
...
template <typename type>
vec<type> operator*(const mat<type> &matrix, const vec<type> &vector) {
  vec<type> solution(vector.size());
...
template <typename type>
vec<type> mat<type>::operator()(const vec<type> &vector) const {
  return ((*this) * vector);
}
...
template <typename type> type *&mat<type>::operator[](uint index) const {
  return comp[index];
}
...
template <typename type>
mat<type> mat_minor(const mat<type> &matrix, uint row, uint column) {
...
template <typename type>
vec<type> solve_cramer(const mat<type> &matrix, const vec<type> &vector) {
  assert(matrix.rows() == matrix.columns());
  assert(matrix.columns() == vector.size());
...
template <typename type>
vec<type> solve_LU(const mat<type> &matrix, const vec<type> &vector) {
  assert(matrix.rows() == matrix.columns());
  assert(matrix.columns() == vector.size());
...
template <typename type>
vec<type> solve_gauss(const mat<type> &matrix, const vec<type> &vector) {
  ...
  for (uint i = 0; i < matrix.columns(); i++) {
    ...
    for (uint j = 0; j < matrix.rows(); j++) {
      ...
      for (uint k = 0; k < matrix.columns(); k++) {
        matrix_temp[j][k] = matrix_temp[j][k] - matrix_temp[temp][k] /
                                                    matrix_temp[temp][i] *
                                                    factor;
      }
      vector_temp[j] =
          vector_temp[j] - vector_temp[temp] / matrix_temp[temp][i] * factor;
    }
  }
